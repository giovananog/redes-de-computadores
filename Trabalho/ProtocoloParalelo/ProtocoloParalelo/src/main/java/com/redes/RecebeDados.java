...
public class RecebeDados extends Thread {
    ...
    private final double PROBABILIDADE_PERDA = 0.6;
    private Random random = new Random();

    private int proximoNumeroSequenciaEsperado = 0;

    private void enviaAck(int numeroSequencia, boolean fim) {
        try {
            ...
            try (FileOutputStream fileOutput = new FileOutputStream("saida")) {
                ...
                while (!fim) {
                    ...
                    if (numeroSequencia == proximoNumeroSequenciaEsperado) {
                        // Lógica para processar os dados recebidos e escrever no arquivo
                        ...
                        // Envio do ACK com o número de sequência do pacote recebido
                        System.out.println("[->] ack: " + numeroSequencia);
                        enviaAck(numeroSequencia, fim);

                        // Atualiza o próximo número de sequência esperado
                        proximoNumeroSequenciaEsperado++;
                    } else {
                        // Pacote fora de ordem, não envia ACK, aguarda o pacote correto
